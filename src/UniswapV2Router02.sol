...
contract UniswapV2Router02 {
    ...
    function _swap_noFee(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        console.log("swap_noFee");
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            console.log("input", input, "output", output);
            (address token0,) = UniswapV2Library.sortTokens(input, output);
            ...
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? UniswapV2Library.pairFor(factory, output, path[i + 2]) : _to;
            UniswapV2Pair(UniswapV2Library.pairFor(factory, input, output)).swap_noFee(
                amount0Out, amount1Out, to, new bytes(0)
            );
        }
        console.log("swap_noFee ending");
    }
    function swapExactTokensForTokens(
        ...
        console.log("swapExactTokensForTokens beginning");
    ...
    function swapExactTokensForTokens_noFee(
        uint amountIn,
        uint amountOutMin,
        address[] calldata path,
        address to,
        ...
        console.log("swapExactTokensForTokens_noFee beginning");
        amounts = UniswapV2Library.getAmountsOut_noFee(factory, amountIn, path);
        console.log(amounts[amounts.length - 1], amountOutMin);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap_noFee(amounts, path, to);
    }
    ...
    function swapTokensForExactTokens_noFee(
        uint amountOut,
        uint amountInMax,
        address[] calldata path,
        address to,
        ...
        amounts = UniswapV2Library.getAmountsIn_noFee(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
